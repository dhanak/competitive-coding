...
int main()
{
	...
	for(uint i=0; i<sources.size(); ++i)
	{
		...
		for(uint d=0; d<4; ++d)
		{
			...
			if(!valid_pos(p))
				continue;
			// a szélén vagyunk és a szélén mozgunk
			// ráadásul nem a target felé, d ezt elintézem annyival
			// hogy nem last candidate
			bool on_border= (s.p.x==0 || s.p.x==X-1) && p.x==s.p.x
				|| (s.p.y==0 || s.p.y==Y-1) && p.y==s.p.y;
			if(on_border && !is_lc)
				continue;
			dm^=1<<d;
		}
