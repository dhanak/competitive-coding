...
bool empty_rectangle(int xb, int yb, int xe, int ye)
{
	for(int y=yb; y<ye; ++y)
		for(int x=xb; x<xe; ++x)
			if(map_in[y][x]!='.')
...
// nem lehet benne target, vagy fel nem használt source
// más lehet (üres, felhasznált source, rajzolt csík)
bool dynamic_empty_rectangle(int xb, int yb, int xe, int ye)
{
	for(int y=yb; y<ye; ++y)
		for(int x=xb; x<xe; ++x)
		{
			if(map[y][x]==0)
				...
				return false;
			else if(1000<=map[y][x] && map[y][x]<2000)
			{
				uint sid=map[y][x]-1000;
				assert(sid<S);
				if(rs.sm&(1ull<<sid))
					continue;
				else return false;
			}
			else if(2000<=map[y][x])
				continue;
			else assert(0);
		}
	return true;
...
bool rec(uint k=0)
{
	...
	for(uint i=k; i<ru.size(); ++i)
	{
		...
		//! ha már úgysem mehet egy irányba akkor nem kéne erőltetni a téglalapokat sem
		// empty rectangle on the top-left,top-right,bottom-right,bottom-left
		bool er[4]=
		{
			dynamic_empty_rectangle(0,0,s.p.x,s.p.y),
			dynamic_empty_rectangle(s.p.x+1,0,X,s.p.y),
			dynamic_empty_rectangle(s.p.x+1,s.p.y+1,X,Y),
			dynamic_empty_rectangle(0,s.p.y+1,s.p.x,Y)
		};
		pos vt=target-s.p;
		for(uint d=0; d<4; ++d)
		{
...
void pre()
{
	...
	for(uint i=0; i<sources.size(); ++i)
	{
		...
		bool er[4]=
		{
			empty_rectangle(0,0,s.p.x,s.p.y),
			empty_rectangle(s.p.x+1,0,X,s.p.y),
			empty_rectangle(s.p.x+1,s.p.y+1,X,Y),
			empty_rectangle(0,s.p.y+1,s.p.x,Y)
		};
		uint dm=0,nvd=0;
		for(uint d=0; d<4; ++d)
		{
