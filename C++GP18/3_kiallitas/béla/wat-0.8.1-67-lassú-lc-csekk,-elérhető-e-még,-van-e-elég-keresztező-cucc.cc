...
bool can_be_finished()
{
	...
	{
		uint lc=lcs[l];
		source const &s=sources[lc];
		pos v=cw4[s.td];
		// olyan üres cella ami keresztből már nem tölthető fel
		...
		for(pos p=target-v; n0<s.n && p!=s.p; p-=v)
		{
			...
			if(map[p.y][p.x]==0)
			{
				if((s.td%2==0?csdy[p.y]:csdx[p.x])==0)
					++n0;
			}
			else if(map[p.y][p.x]<2000)
			{
				uint sid=map[p.y][p.x]-1000;
				assert(sid<S);
				// hmm egy fel nem használt source útközben,
				// ez egy csomó cellát le tudna fedni
				// nem gondolkozok hogy mennyit tudna, feltételezem hogy még
				// megoldható vele
				if(((rs.sm>>sid)&1)==0)
					return true;
			}
		...
		if(n0<s.n)
			return true;
		// fel nem használt source a csekkelten túl
		for(pos p=s.p-v; valid_pos(p); p-=v)
		{
			if(1000<=map[p.y][p.x] && map[p.y][p.x]<2000)
			{
				uint sid=map[p.y][p.x]-1000;
				assert(sid<S);
				if(((rs.sm>>sid)&1)==0)
