...
def load_solution_from_file(file_path):
    ...
    for line in lines:
        ...
        positions.append(Position(x - 1, y - 1))
    return positions
...
def run_custom_checker(t_obj, r_obj):
    ...
    try:
        ...
        r_obj.result = False;
        r_obj.score = 0.0;
        r_obj.message = "Output format does not satisfy the requirements...";
        return
    
    if len(solution) != len(pieces):
        r_obj.result = False;
        r_obj.score = 0.0;
        r_obj.message = "Incorrect number of pieces used...";
        return
    ...
    r_obj.result = True;
    r_obj.score = pow((board.get_min_score() - board.get_score()), 3) / pow(board.get_min_score(), 3);
...
class T_obj:
    ...
    def __init__(self, tip, top):
        self.testcase_input_path = tip
        self.testcase_output_path = top
...
if __name__ == "__main__":
    ...
    a = T_obj(args.input_file, args.output_file)
    b = R_obj()
    
    run_custom_checker(a, b)
